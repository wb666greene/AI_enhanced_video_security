...
if __WIN__ == False:
    # NCS v1SDK can be copied from a Pi of the same model to the OpenVINO system.
    # Definite issues mixing NCS & NCS2
#OV##    from mvnc import mvncapi as mvnc
    import signal
...
if cv2.__version__  != '4.0.1-openvino':     # we need "special" Intel OpenVINO version of OpenCV!
    print("Need Intel verison of OpenCV: 4.0.1-openvino")
...
#******************************************************************************************************************
#******************************************************************************************************************
if False:   #OV## disable NCS SDK code for initial OpenVINO attempt
#OV##if __WIN__ is False:
...
## *** OpenVion Thread ***  looks exactly like the CPU_AI_thread, differences are in the net arguement and my [INFO] print statements
#******************************************************************************************************************
#******************************************************************************************************************
def OpenVINO_AI_thread(results, inframe, net, tnum, camearaLock):
    global nextCamera
    global waitThreadSync
    global QUIT
    global Ncameras
    while waitThreadSync and not QUIT:
        time.sleep(0.10)
    print("[INFO] OpenVINO AI thread" + str(tnum) + " is running...")
    bcnt=0
    mcnt=0
    ...
    while not QUIT:     # I'm not sure later versions fix it, or if its confined to the Raspberry Pi camera module, but I've left it in here.
        ...
        try:
            ...
            continue
    # Thread exits
    print("OpenVINO AI thread" + str(tnum) + " is exiting, waited for input frames " + str(bcnt) + " times, dropped " + str(mcnt) + " output frames.")
...
def main():
    ...
    ap.add_argument("-nNCS", "--nNCS", type=int, required=True, help="number of Myraid devices")
    ...
    #OV##ap.add_argument("-noNCS", "--noNCS", action="store_true", help="don't probe for Movidius NCS")
...
#OV##        else:
#OV##            print("*** No NCS devices found and CPU threads turned off in startup option! --- Exiting...")
#OV##            quit()
    if nCPUthreads > 0:
    ...
    nOVthreads = nNCS 
    if nOVthreads > 0:
        print("[INFO] loading Caffe model for OpenVINO AI threads...")
        netOV=list()
        for i in range(nOVthreads):
            print("[INFO] loading model...")
            netOV.append(cv2.dnn.readNetFromCaffe("MobileNetSSD_deploy.prototxt", "MobileNetSSD_deploy.caffemodel"))
    ...
    OVt = list()
    if nOVthreads > 0:
        print("[INFO] starting " + str(nOVthreads) + " OpenVINO AI Threads ...")
        for i in range(nOVthreads):
            OVt.append(Thread(target=OpenVINO_AI_thread, args=(results, inframe, netOV[i], i, cameraLock)))
            OVt[i].start()
    ...
    if nOVthreads > 0:
        for i in range(nOVthreads):
            if not results.empty():
                (_, _, _) = results.get(False)
            OVt[i].join()
        print("[INFO] All OpenVINO AI Threads have exited ...")
